...
def get_training_augmentations(aug_type, resize_size=256, crop_size=224):
    ...
    if 'soft' in aug_type:
        ...
        augmentations.extend([A.augmentations.transforms.GaussianBlur(sigma_limit=(0.0, 3.0), p=0.5),
                              A.augmentations.transforms.ImageCompression(quality_lower=30, quality_upper=100, p=0.5)])
    elif 'oneof' in aug_type:
        augmentations.append(A.OneOf([A.augmentations.transforms.GaussianBlur(sigma_limit=(0.0, 3.0), p=0.5),
                                      A.augmentations.transforms.ImageCompression(quality_lower=30, quality_upper=100, p=0.5),
                                      A.augmentations.transforms.ISONoise(p=0.5),
                                      A.augmentations.transforms.ColorJitter(0.4, 0.4, 0., 0., p=0.5)]))
    elif 'strong' in aug_type:
        augmentations.append(A.SomeOf([A.augmentations.transforms.GaussianBlur(sigma_limit=(0.0, 3.0), p=0.5),
                                       A.augmentations.transforms.ImageCompression(quality_lower=30, quality_upper=100, p=0.5),
                                       A.augmentations.transforms.ISONoise(p=0.5),
...
def get_validation_augmentations(resize_size=256):
    return A.Compose([
